      ...
      <div class="col-sm-6 offset-3">
        ...
        <div v-if="sessionStarted" id="chat-container" class="card">
          <div class="card-header text-white text-center font-weight-bold subtle-blue-gradient">
            Скопируйте URL что бы открыть комнату для еще одного пользователя
          </div>
        ...
        <div v-else>
          <h3 class="text-center">Здравствуйте {{ username }}!</h3>
          <br />
          <p class="text-center">
            Чтобы начать общение, нажмите на кнопку ниже,
            она запустит новый сеанс чата, а затем вы сможете добавить
            пользователя в чат!
          </p>
          <br />
          <button @click="startChatSession"
                  class="btn btn-primary btn-lg btn-block">Войти в чат</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    connectToWebSocket () {
      const websocket = new
      WebSocket(`ws://localhost:5000/${this.$route.params.uri}`)
...
.subtle-blue-gradient {
  background: linear-gradient(45deg, #00a7ff, #007bff);
}
